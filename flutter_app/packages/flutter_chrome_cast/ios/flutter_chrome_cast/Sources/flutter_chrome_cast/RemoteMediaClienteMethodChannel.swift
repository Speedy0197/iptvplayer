...
class RemoteMediaClienteMethodChannel :UIResponder, FlutterPlugin, GCKRemoteMediaClientListener{
    ...
    private var requests: [ObjectIdentifier: CastRequestWaiter] = [:]

    private func awaitRequest(_ request: GCKRequest?, result: @escaping FlutterResult) {
        guard let request = request else {
            result(FlutterError(code: "CAST_NO_CLIENT", message: "No active Cast media client.", details: nil))
            return
        }
        let key = ObjectIdentifier(request)
        let waiter = CastRequestWaiter(request: request, result: result) { [weak self] in
    ...
    private  func loadMedia(_ arguments : Dictionary<String,Any>, result: @escaping FlutterResult )  {
        ...
        awaitRequest(request, result: result)
    ...
    private func play(_ result: @escaping FlutterResult){
    ...
    private func seek(_ result: @escaping FlutterResult, _ args : Dictionary<String, Any>){
...
/// Retains the native request until acknowledged, failed, aborted or timed out.
private final class CastRequestWaiter: NSObject, GCKRequestDelegate {
    private let request: GCKRequest
    private var result: FlutterResult?
    private let finished: () -> Void
    private var timer: Timer?

    init(request: GCKRequest, result: @escaping FlutterResult, finished: @escaping () -> Void) {
    ...
    func start() {
        ...
        timer = Timer.scheduledTimer(withTimeInterval: 10, repeats: false) { [weak self] _ in self?.fail() }
        if !request.inProgress { complete(success: request.error == nil) }
    }
    ...
    private func complete(success: Bool) {
        ...
        if success {
            callback(["inProgress": false, "isExternal": request.external, "requestID": request.requestID])
        } else {
            callback(FlutterError(code: "CAST_REQUEST_FAILED", message: "The Cast request did not complete.", details: nil))
        }
        finished()
    }
}
